  ...
  SELECT a.assign_date , a.follow_id ,a.customer_type , a.cus_intention_type, b.invite_code flowUpInviteCode,b.name followUp, c.id, c.link_person , c.bank_name , c.create_date , c.card_valid_date ,c.bank_sign_date , c.mobile, c.bank_sign , c.record_invite_code , c.id_card , IFNULL(d.total_in_cash, 0) inCash, d.frist_in_cash_date , IFNULL(d.useable + d.frozen + d.out_frozen + IFNULL(e.amount, 0), 0) totalFund, IFNULL(f.outCash, 0) dayOutCash, IFNULL(f.inCash, 0) dayInCash, f.dayLastInCashTime, f.dayLastOutCashTime from follow_agent a

	left join ( select id,invite_code,name,disable from follow_up )b on a.follow_id=b.id

	left join ( select id, link_person , bank_name , create_date , card_valid_date ,bank_sign_date , mobile, bank_sign , record_invite_code ,id_card, account_id,uc_id,bank_account from dev_dayi_spot.agent where del_status != -1 )c on c.id = a.agent_id

	left join dev_dayi_spot.finance_account d on d.id = c.account_id

	left join ( SELECT agent, IFNULL(SUM(amount), 0) amount from dev_dayi_spot.protocol WHERE status in (1,2) GROUP BY agent )e on e.agent=c.id

	left join ( SELECT pay_uc_id, MAX( CASE WHEN pay_type = '1' THEN accrual END ) AS inCash, MAX(CASE WHEN pay_type = '-114' THEN accrual END ) AS outCash, MAX(CASE WHEN pay_type = '1' THEN createDate END ) AS dayLastInCashTime, MAX(CASE WHEN pay_type = '-114' THEN createDate END ) AS dayLastOutCashTime FROM ( SELECT pay_uc_id,`pay_type`,SUM(accrual) accrual,MAX(`create_date`)createDate FROM dev_dayi_spot.finance_account_log WHERE create_date BETWEEN ? and ? AND pay_type IN (1,-114) and account_type = 0 GROUP BY pay_uc_id,`pay_type` )f1 GROUP BY `pay_uc_id` )f ON c.uc_id = f.pay_uc_id

	left join ( SELECT agent_id,GROUP_CONCAT( bank_id ORDER BY bank_id ) bank_ids FROM dev_dayi_spot.finance_vitrual_account GROUP BY agent_id )g ON c.id = g.agent_id

WHERE 1 = 1 and b.disable != -1   AND b.id =36   AND ifnull(d.total_in_cash,0) > 0 ORDER BY a.assign_date DESC LIMIT 0 , 20
